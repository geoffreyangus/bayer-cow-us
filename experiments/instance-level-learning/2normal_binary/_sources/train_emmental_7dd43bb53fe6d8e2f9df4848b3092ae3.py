...
def config(transforms):
    ...
    """
    hypothesis_conditions = ['instance-level-learning', '2normal_binary']
...
class TrainingHarness(object):
    ...
    def _init_dataloaders(self, _log, dataloader_configs, sampler_configs, task_to_label_dict):
        ...
        for split in ['train', 'valid']:
            ...
            if split == 'train':
                dataloader_config = {
                    'sampler': getattr(torch_data, sampler_configs[split]['class_name'])(
                                       data_source=self.datasets[split], **sampler_configs[split]['args']),
                    **dataloader_config
                }
